  ...
  return d3.json("ca-all-all.geo.json", function(json) {
    ...
    svg.selectAll("path")
       ...
       .attr("county-name", function(province) {
         var cn = province.properties.name.toLowerCase().split(" ").join("-").split(".").join("").split("'").join("").split(", ").join("-")
         return cn
       })
       ...
       .on("mouseover", function(province) {
         ...
         var occurrences = document.querySelectorAll(`[county-name=${province.properties.name.toLowerCase().split(" ").join("-").split(".").join("").split("'").join("").split(", ").join("-")}]`).length

         d3.select("div#infobox")
           .text(`${countyName} ${type}, ${provinceName} (${occurrences} occurrences)`)

         d3.selectAll(`[county-name=${province.properties.name.toLowerCase().split(" ").join("-").split(".").join("").split("'").join("").split(", ").join("-")}]`)
           .attr("fill", "white")
       })
